...
from torch_rechub.utils.match import generate_seq_feature_match_v0, gen_model_input, generate_seq_feature_match
...
def get_movielens_data(data_path, load_cache=False):
    ...
    np.save("/root/code/torch-rechub/examples/matching/data/ml-1m/saved/raw_id_maps.npy", np.array((user_map, item_map), dtype=object))
    ...
    if load_cache:  #if you have run this script before and saved the preprocessed data
        x_train, y_train, x_test = np.load("/root/code/torch-rechub/examples/matching/data/ml-1m/saved/data_cache.npy", allow_pickle=True)
    else:
        ...
        np.save("/root/code/torch-rechub/examples/matching/data/ml-1m/saved/data_cache.npy", np.array((x_train, y_train, x_test), dtype=object))
...
def main(dataset_path, model_name, epoch, learning_rate, batch_size, weight_decay, device, save_dir, seed):
    ...
    user_features, item_features, neg_item_feature, x_train, y_train, all_item, test_user = get_movielens_data(dataset_path, load_cache=True)
    ...
    match_evaluation(user_embedding, item_embedding, test_user, all_item, user_col='user_id', item_col='movie_id', raw_id_maps="/root/code/torch-rechub/examples/matching/data/ml-1m/saved/raw_id_maps.npy", topk=200)


if __name__ == '__main__':
    import argparse
    parser = argparse.ArgumentParser()
    parser.add_argument('--dataset_path', default="/root/autodl-tmp/ml-1m.csv")
